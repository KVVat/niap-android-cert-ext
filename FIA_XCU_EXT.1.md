...
## Why mTLS Is Required — Rationale

### Background: What Android Normally Does

Standard Android TLS is one-way (server authentication only). The device verifies the server's
certificate, but the server has no way to verify the identity of the device at the TLS layer.
Authentication of the client (if any) is deferred to the application layer — e.g., username/password,
session tokens, or API keys.

This is sufficient for general consumer use, but **insufficient for NIAP/Common Criteria compliance**,
where the Target of Evaluation (TOE) — the device itself — must be able to assert its own identity
...
```
FIA_XCU_EXT.2.1
  The TSF shall request certificates from an external CA
  to represent TOE functions for TLS.
        │
        │  → The device must obtain a certificate that identifies it
        │    as a specific, registered entity (not an anonymous client)
        ▼
FIA_X509_EXT.3.1
  The TSF shall generate Certificate Requests using RFC 7030 (EST).
        │
        │  → EST (Enrollment over Secure Transport) is the required
...
FIA_X509_EXT.1.5
  ...
  Client Authentication purpose (OID 1.3.6.1.5.5.7.3.2).
        │
        │  → The acquired certificate must carry the clientAuth EKU,
        │    meaning it is explicitly intended for TLS client auth
        ▼
FIA_X509_EXT.2.1
  The TSF shall validate X.509v3 certificates to support TLS.
        │
        │  → The certificate must be actively used in TLS — i.e.,
        │    presented to the server during the TLS handshake (mTLS)
        ▼
  Device identity is established at the TLS layer ✅
```

### Why mTLS Specifically

In ordinary TLS, the client certificate is never sent — the server accepts connections from anyone
and relies on higher-layer authentication. In mutual TLS (mTLS), the server explicitly requests the
client's certificate during the TLS handshake and verifies it against a trusted CA.

This distinction matters for NIAP because:

| | Standard TLS | mTLS |
|--|-------------|------|
| Server verified by client | ✅ | ✅ |
| Client verified by server | ❌ | ✅ (at TLS layer) |
| Identity guarantee | Application layer only | Transport layer |
| Satisfies FIA_XCU_EXT.2.1 | ❌ | ✅ |

Without mTLS, the certificate acquired via EST would sit unused in the KeyStore. The NIAP requirement
is that the certificate is *used* to represent TOE functions in authenticated communications —
which specifically means presenting it in a TLS handshake.

### EST Server Certificate Requirement

The EST server itself is also subject to certificate requirements (FIA_X509_EXT.1.5):

> Server certificates presented for EST shall have the CMC Registration Authority (RA) purpose  
> (id-kp-cmcRA with OID 1.3.6.1.5.5.7.3.28)

This ensures that the CA issuing the client certificate is a properly identified Registration Authority,
not an arbitrary HTTPS server. A client that validates this EKU can confirm it is enrolling with a
legitimate EST RA, not a spoofed endpoint.

### Summary

The implementation in this project (EST enrollment → AndroidKeyStore → mTLS via signing oracle)
is the Android-native realization of the following NIAP requirement:

> A NIAP-compliant device must be able to acquire an X.509 certificate from an external CA using
> EST, and use that certificate to authenticate itself at the TLS layer (mTLS) when accessing
> protected resources.

Android does not provide this capability out of the box — the platform KeyChain API supports
key storage but does not expose the EST enrollment workflow or the cross-process signing oracle
needed to perform mTLS without exposing private key material to the calling application.
